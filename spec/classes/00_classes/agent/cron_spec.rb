  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      context 'with facts set to defaults' do
        ...
        context 'with default params' do
          ...
          it 'uses maxruntime to kill processes in puppetagent_cron.sh' do
            ...
            is_expected.to contain_file('/usr/local/bin/puppetagent_cron.sh').with_content(%r{#{expected}})
          end

          it 'includes code to break an existing puppet lock in puppetagent_cron.sh' do
            is_expected.to contain_file('/usr/local/bin/puppetagent_cron.sh').with_content(%r{handles forcibly enabling puppet agent})
          end
        ...
        context "with 'sha256' randomization algorithm for minute" do
          ...
          it {
            is_expected.to contain_systemd__timer('puppet_agent.timer')
              .with_timer_content(%r{OnCalendar=\*-\* \*:10,40})
              .with_service_content(%r{ExecStart=/usr/local/bin/puppetagent_cron.sh})
              .with_service_content(%r{SuccessExitStatus=2})
              .that_requires('File[/usr/local/bin/puppetagent_cron.sh]')
          }
        ...
        context 'with alternate minute_base' do
          ...
          it {
            is_expected.to contain_systemd__timer('puppet_agent.timer')
              .with_timer_content(%r{OnCalendar=\*-\* \*:29,59})
              .with_service_content(%r{ExecStart=/usr/local/bin/puppetagent_cron.sh})
              .with_service_content(%r{SuccessExitStatus=2})
              .that_requires('File[/usr/local/bin/puppetagent_cron.sh]')
          }
        ...
        context 'with specific cron parameters specified' do
          ...
          it {
            is_expected.to contain_systemd__timer('puppet_agent.timer')
              .with_timer_content(%r{OnCalendar=Fri 4-3 2:1})
              .with_service_content(%r{ExecStart=/usr/local/bin/puppetagent_cron.sh})
              .with_service_content(%r{SuccessExitStatus=2})
              .that_requires('File[/usr/local/bin/puppetagent_cron.sh]')
          }
        ...
        context 'with break_puppet_lock disabled' do
          ...
          it { is_expected.to contain_file('/usr/local/bin/puppetagent_cron.sh').with_content(%r{handles puppet processes which have been running longer than maxruntime}) }
          it { is_expected.not_to contain_file('/usr/local/bin/puppetagent_cron.sh').with_content(%r{handles forcibly enabling puppet agent}) }
        end
      ...
      context 'with puppet service enabled' do
        ...
        it 'execs script to disable puppet service' do
